...
void loop() {
 ...
 dist_ema = _EMA_ALPHA*dist_filtered + (1.0 -_EMA_ALPHA)*dist_ema;
  ...
  float d_mm = constrain(dist_ema, _DIST_MIN, _DIST_MAX);

  // 2) 정수 map() 사용을 위해 mm를 long으로 반올림 변환
  long d_mm_i = (long)(d_mm + 0.5f);

  // 3) 180mm → 0°, 360mm → 180° 로 선형 매핑
  long angle = map(d_mm_i, (long)_DIST_MIN, (long)_DIST_MAX, 0, 180);

  // 4) 각도(0~180)를 서보 펄스폭(_DUTY_MIN~_DUTY_MAX)으로 매핑
  long duty_us = map(angle, 0, 180, _DUTY_MIN, _DUTY_MAX);
